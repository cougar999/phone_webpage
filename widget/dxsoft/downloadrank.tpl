	...
	<div class="ranklist clear">
		...
		<dl>
			...
			<ul>
				...
				<li class="downBtn"><div class="down f_l"><a id="<!--{$item.fileId}-->" businesstype="1" isbiz=<!--{$isbiz}--> href="<!--{$item.path}-->&isbiz=<!--{$isbiz}-->" installlocate="1" appid="<!--{$item.packageName}-->"  versioncode="<!--{$item.versionCode}-->"  version="<!--{$item.versionName}-->" title="<!--{$item.name}-->" star="<!--{$item.star}-->" type="1" goldcoins="1" class="change downinit newpage"  onclick="return false;"></a></div></li>
			</ul>
		</dl>
		<!--{/foreach}-->
	</div>
