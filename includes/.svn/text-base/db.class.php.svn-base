...
class db {
	...
	function connect($db_config){
		if ($this->pconnect){
			...
			$this->connection_id = @mysql_connect($db_config["hostname"], $db_config["username"], $db_config["password"]) or die("Can not connect MySQL Server". mysql_error());
		}
	...
	function sql_select($tbname, $where="", $limit=0, $page=0, $fields="*", $orderby="id", $sort="DESC"){
		$sql = "SELECT ".$fields." FROM `".$tbname."` ".($where?" WHERE ".$where:"")." ORDER BY ".$orderby." ".$sort.($limit ? " limit ".($page-1)*$limit.",".$limit:"");
		return $sql;
	}
	 ...
	 */
	function row_select($tbname, $where="", $limit=0, $page=0, $fields="*", $orderby="id", $sort="DESC"){
		$sql = $this->sql_select($tbname, $where, $limit, $page, $fields, $orderby, $sort);
